...
* **feat(imports)** ([#20](https://github.com/utakata-code/utakata/issues/20)): New `utakata imports` — a deterministic import-health audit. `arch_definition.yaml` gains a machine-readable `import_rules` section that formalizes what each layer guide's `allowed_imports`/`forbidden_imports` only illustrated: an internal **whitelist** (which layers each layer directory may import — the own layer is always allowed, and paths belonging to no layer such as `core/` or `main.dart` are exempt, which is where composition roots live) and a per-layer external **blacklist** (package-name globs like `*riverpod*`, `dart:` URIs too; multiple matching rules stack), plus `exclude` globs for generated files (`**.g.dart`). `package:<your app>/...` imports are resolved as internal, same as relative ones. Violations exit 1; `--json` for CI and AI agents; architectures without `import_rules` are skipped with a notice. The bundled `clean_architecture`/`mvvm` definitions ship with rules consistent with their guides. New doc topic: `utakata doc show imports`.
