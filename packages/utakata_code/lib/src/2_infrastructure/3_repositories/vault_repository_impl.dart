...
class VaultRepositoryImpl implements VaultRepository {
  ...
  Future<VaultRef?> _resolveRef(String projectDir) async {
    final fromProject = (await _configRepo.read(projectDir))?.vault;
    if (fromProject != null && !fromProject.isEmpty) return fromProject;
    final fromGlobal = (await _configRepo.readGlobal())?.vault;
    if (fromGlobal != null && !fromGlobal.isEmpty) return fromGlobal;
    return null;
  }
  ...
  Future<String?> resolveRoot(String projectDir) async {
    final ref = await _resolveRef(projectDir);
    if (ref == null) return null;

    final localPath = ref.path;
    if (localPath != null && localPath.isNotEmpty) {
      final expanded = _expandHome(localPath);
