...
このワークフローは、プロジェクトの現在状態を検出して `utakata/snapshots/project_status.yaml` を自動更新します。
...
1. ステータスファイルを更新
```bash
./utakata/scripts/status/update_status.sh --yes
```
...
```bash
./utakata/scripts/status/update_status.sh
```
