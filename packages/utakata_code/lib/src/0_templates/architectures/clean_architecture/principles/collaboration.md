...
2. 影響するフィーチャーを doc/specs/plan.yaml に反映
3. utakata plan で計画書を再生成
4. 実装を進める
5. utakata check でゼロ違反を確認
```

### 新機能追加時
```
1. doc/specs/plan.yaml に追記
2. utakata plan
...
### コード変更後に必ず実行
```bash
utakata check   # 命名規則・構造違反がゼロであること
```
...
### AIの出力を受け取ったら
- `utakata check` を実行して命名規則・構造を確認する
